...
def three_layer_convnet(x, params):
    ...
    ############################################################################
    s = [1, 1, 1, 1]
    layer1_custom_padding = tf.constant([[0,0], [2, 2], [2, 2], [0, 0]])
    layer2_custom_padding = tf.constant([[0,0], [1, 1], [1, 1], [0, 0]])
    
    new_x = tf.pad(x, layer1_custom_padding, 'CONSTANT')
    layer1_conv = tf.nn.conv2d(new_x, conv_w1, strides = s, padding = "VALID") + conv_b1
    layer1_relu = tf.nn.relu(layer1_conv)
    
    new_x = tf.pad(layer1_conv, layer2_custom_padding, 'CONSTANT')
    layer2_conv = tf.nn.conv2d(new_x, conv_w2, strides = s, padding = "VALID") + conv_b2
    layer2_relu = flatten(tf.nn.relu(layer2_conv))
    scores = tf.matmul(layer2_relu, fc_w) + fc_b
#     pass
    ############################################################################
    #                              END OF YOUR CODE                            #
    ############################################################################
...
def three_layer_convnet_init():
    ...
    ############################################################################
    layer1_conv_w = tf.Variable(kaiming_normal([5, 5, 3, 32]))
    layer1_conv_b = tf.Variable(np.zeros([32]), dtype=tf.float32)
    layer2_conv_w = tf.Variable(kaiming_normal([3, 3, 32, 16]))
    layer2_conv_b = tf.Variable(np.zeros([16]), dtype=tf.float32)
    fc_w = tf.Variable(kaiming_normal([32*32*16,10]))
    fc_b = tf.Variable(np.zeros([10]), dtype=tf.float32)
    params = (layer1_conv_w, layer1_conv_b, layer2_conv_w, layer2_conv_b, fc_w, fc_b)
#     pass
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        ########################################################################
        TLC_initializer = tf.variance_scaling_initializer(scale = 2.0)
        self.layer1_conv = tf.layers.Conv2D(channel_1, [5,5], [1,1], padding = 'valid', kernel_initializer = TLC_initializer, activation=tf.nn.relu)
        self.layer2_conv = tf.layers.Conv2D(channel_2, [3,3], [1,1], padding = 'valid', kernel_initializer = TLC_initializer, activation = tf.nn.relu)
        self.layer_fc = tf.layers.Dense(num_classes, kernel_initializer = TLC_initializer)
#         pass
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
    ...
    def call(self, x, training=None):
        ...
        layer1_custom_padding = tf.constant([[0,0], [2,2], [2,2], [0,0]])
        layer2_custom_padding = tf.constant([[0,0],[1,1],[1,1],[0,0]])
        x = tf.pad(x, layer1_custom_padding, 'CONSTANT')
        x = self.layer1_conv(x)
        x = tf.pad(x, layer2_custom_padding, 'CONSTANT')
        x = self.layer2_conv(x)
        x = tf.layers.flatten(x)
        scores = self.layer_fc(x)
#         pass
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################        
...
def model_init_fn(inputs, is_training):
    ...
    initializer = tf.variance_scaling_initializer(scale = 2.0)
    layers = [
        tf.keras.layers.InputLayer(input_shape = inp_dim), tf.keras.layers.Conv2D(layer1_channel, [5,5], [1,1], padding = 'SAME', kernel_initializer = initializer, activation = tf.nn.relu),
        tf.keras.layers.Conv2D(layer2_channel, [3,3], [1,1], padding = 'same', kernel_initializer = initializer, activation = tf.nn.relu),
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(classes_cnt, kernel_initializer = initializer)
    ]
    model = tf.keras.Sequential(layers)
#     pass
    ############################################################################
...
def model_init_fn(inputs, is_training):
    ...
    # Layer 1
    layer1_conv = tf.layers.conv2d(inputs, 32, [3,3], [1,1], padding = 'same', kernel_initializer = initializer)
    layer1_batchnorm = tf.layers.batch_normalization(layer1_conv, training = is_training)
    layer1_relu = tf.nn.relu(layer1_batchnorm)
    layer1_drop = tf.layers.dropout(layer1_relu)
    layer1_maxpool = tf.layers.max_pooling2d(layer1_drop, [2,2], [2,2])
    
    #Layer 2
    layer2_conv = tf.layers.conv2d(layer1_maxpool, 64, [3,3], [1,1], padding = 'valid', kernel_initializer = initializer)
    layer2_batchnorm = tf.layers.batch_normalization(layer2_conv, training = is_training)
    layer2_relu = tf.nn.relu(layer2_batchnorm)
    layer2_drop = tf.layers.dropout(layer2_relu)
    layer2_maxpool = tf.layers.max_pooling2d(layer2_drop, [2,2], [2,2])
    
    #Layer 3
    layer3_conv = tf.layers.conv2d(layer2_maxpool, 128, [3,3], [1,1], padding = 'valid', kernel_initializer = initializer)
    layer3_batchnorm = tf.layers.batch_normalization(layer3_conv, training = is_training)
    layer3_relu = tf.nn.relu(layer3_batchnorm)
    layer3_drop = tf.layers.dropout(layer3_relu)
    
    #Layer 4
    avg_pool = tf.layers.average_pooling2d(layer3_drop, [5,5], [1,1])
    new_avg_pool = tf.layers.flatten(avg_pool)
    
    #Layer 5
    layer1_fc = tf.layers.dense(new_avg_pool, 50)
    
    #Layer 6
    layer2_fc = tf.layers.dense(layer1_fc, 50)
    net = tf.layers.dense(layer2_fc, class_cnt)
#     pass
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
